    ...
    <script type="text/javascript">
        ...
        function changeRecordsPerPage(selectElement) {
            ...
            window.location.href = `?page=1&search=<?php echo htmlspecialchars($searchValue); ?>&limit=${selectedValue}`;
                                ...
                                $sql1 = "SELECT COUNT(*) AS total FROM (SELECT cp.Bill_No FROM crm_invoice AS cp
                                            INNER JOIN crm_contact AS cc ON cp.Cust_Id = cc.id
                                            WHERE cp.Cust_Id = cc.id AND cc.Emp_Email = '$eml' 
                                            AND (cp.Bill_No LIKE '%$searchValue%' OR cc.contact_first LIKE '%$searchValue%' OR cc.contact_last LIKE '%$searchValue%' OR cc.company LIKE '%$searchValue%' OR cc.city LIKE '%$searchValue%')
                                            GROUP BY cp.Bill_No) AS TotalCount";
                                ...
                                $sql = "SELECT cp.id, cp.Bill_No, cp.Bill_Date, cp.Cust_Id, cp.Category, cp.Subcategory, cp.Work_Scope, cp.Amt,
                                        ...
                                        FROM crm_invoice AS cp
                                        INNER JOIN crm_contact AS cc ON cp.Cust_Id = cc.id
                                        WHERE cp.Cust_Id = cc.id AND cc.Emp_Email = '$eml' 
                                        AND (cp.Bill_No LIKE '%$searchValue%' OR cc.contact_first LIKE '%$searchValue%' OR cc.contact_last LIKE '%$searchValue%' OR cc.company LIKE '%$searchValue%' OR cc.city LIKE '%$searchValue%')
                                        GROUP BY cp.Bill_No ORDER BY cp.Bill_No LIMIT $offset, $limit";
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<td>" . $row['city'] . "</td>";
                                    echo "<td><a href=\"infoinvoice.php?Bill_No={$row['Bill_No']}\">View</a></td>";
                                    echo "</tr>";
                                    $i++;
                                }
                                echo '</table></center> ';
